...
class BaseAgent:
    ...
    def get_exploration_action(self, observations, targets):
        ...
        noise_tf = tf.convert_to_tensor(self.action_noise.sample() * self.action_noise_factor, dtype=tf.float32)
        return self._get_exploration_action(observations_tf, targets_tf, noise_tf).numpy()

    @tf.function
    def _get_exploration_action(self, observations, targets, noise):
        observations = tf.expand_dims(observations, 0)  # add batch dim
        targets = tf.expand_dims(targets, 0)
        action = tf.squeeze(self.actor([observations, targets]))
        new_action = action + noise
        action_saturated = tf.squeeze(tf.clip_by_value(new_action, -1, 1))
    ...
    def get_exploitation_action(self, observations, targets):
        ...
        observations_tf = tf.convert_to_tensor(observations, dtype=tf.float32)
        targets_tf = tf.convert_to_tensor(targets, dtype=tf.float32)
        action = self._get_exploitation_action(observations_tf, targets_tf)
        return action.numpy()

    @tf.function
    def _get_exploitation_action(self, observations, targets):
        observations = tf.expand_dims(observations, 0)  # add batch dim
        targets = tf.expand_dims(targets, 0)
        action = self.actor([observations, targets])
