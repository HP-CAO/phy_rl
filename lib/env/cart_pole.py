...
class Cartpole(gym.Env):
    ...
    def step(self, action: float, action_mode="residual"):
        ...
        if self.use_linear_model:
            ...
            if action_mode == 'residual':
                F = np.array([8.25691599, 6.76016534, 40.12484514, 6.84742553]) * 1.2
                # F = np.array([16.0700003, 16.98967248, 76.71487216, 28.52953581]) * 0.7
                # F = np.array([15.84304695, 19.022035, 81.22142291, 27.98173083])
                force_res = F[0] * x + F[1] * x_dot + F[2] * theta + F[3] * theta_dot  # residual control commands
