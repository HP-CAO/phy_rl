...
MAX_DDQ = np.array((10., 10., 10., 20., 20., 20.)) * 5.5
MIN_DDQ = -MAX_DDQ
...
class TorqueStanceLegController(leg_controller.LegController):
  ...
  """
    ...
    def get_action(self, drl_action=None, mode="mpc"):
        ...
        robot_q = np.hstack((self.estimate_robot_x_y_z(), robot_com_roll_pitch_yaw))
        ...
        if mode == 'residual':
            ...
            desired_ddq = drl_action * MIN_DDQ
        else:
            desired_ddq = KP * (desired_q - robot_q) + KD * (desired_dq - robot_dq)

        desired_ddq = np.clip(desired_ddq, MIN_DDQ, MAX_DDQ)  # according to this to set the action range
