...
from lib.env.gym_physics import GymPhysics
from lib.utils import states2observations
from lib.env.reward import RewardFcn
...
class ModelStatsParams:
    def __init__(self):
        ...
        self.eval_on_multi_conditions = True
...
class ModelStats:
    ...
    def get_average_actions_std(self):
        if len(self.actions_std) == 0:
            return 0
        else:
            return sum(self.actions_std) / len(self.actions_std)
    ...
    def training_monitor(self, episode):
        ...
        average_reward, on_target_steps, average_distance_score, survived, can_swing_up, swing_up_time, critic_loss, actions_std = self.log_data()
        with self.training_log_writer.as_default():
            ...
            tf.summary.scalar('actions_std', actions_std, self.total_steps)
        current_time = time()

        print("Training:=====>  Episode: ", episode, " total steps:", self.get_steps(),
              f" Avg_reward: {np.round(average_reward, 3):0.3f}",
              f" ds_mean: {np.round(average_distance_score, 3):0.3f}",
              f" eps_duration: {np.round(current_time-self.old_time, 3):0.3f}")
        self.old_time = current_time

    def evaluation_monitor_scalar(self, episode):

        average_reward, on_target_steps, average_distance_score, survived, can_swing_up, swing_up_time, _, actions_std = self.log_data()
    ...
    def multi_evaluation_scalar(self, episode, episode_log_data_list):

        data_arr = np.array(episode_log_data_list)
        reward_arr = data_arr[:, 0]
        on_target_steps_arr = data_arr[:, 1]
        distance_score_arr = data_arr[:, 2]
        survived_arr = data_arr[:, 3]
        can_swing_up_arr = data_arr[:, 4]
        swing_up_time_arr = data_arr[:, 5]
        actions_std = data_arr[:, 7]

        average_distance_score_arr = distance_score_arr.mean()

        swing_up_success_rate = np.count_nonzero(can_swing_up_arr) / len(can_swing_up_arr)

        with self.evaluation_log_writer.as_default():
            tf.summary.scalar('Average_Reward', np.mean(reward_arr), self.total_steps)
            tf.summary.scalar('On_target_step', np.mean(on_target_steps_arr), self.total_steps)
            tf.summary.scalar('swing_up_success_rate', swing_up_success_rate, self.total_steps)
            tf.summary.scalar('swing_up_time', np.mean(swing_up_time_arr), self.total_steps)
            tf.summary.scalar('distance_score', average_distance_score_arr, self.total_steps)
            tf.summary.scalar('distance_score_and_survived', np.mean(average_distance_score_arr * survived_arr), self.total_steps)
            tf.summary.scalar('actions_std', np.mean(actions_std), self.total_steps)

        print("Evaluation:=====>  Episode: ", episode, " Total steps:",
              self.get_steps(), " Successful rate: ", swing_up_success_rate, "ds_mean", average_distance_score_arr)

    def evaluation_monitor_image(self, ep_id):

        average_reward, on_target_steps, average_distance_score, survived, can_swing_up, swing_up_time, _, actions_std = self.log_data()
        tf_image = self.plot_to_image(average_reward, on_target_steps, average_distance_score)

        with self.evaluation_log_writer.as_default():
            tf.summary.image('Summary_plot_{}'.format(ep_id), tf_image, self.total_steps)

    def log_data(self):
        actions_std = self.get_average_actions_std()
        ...
        return average_reward, on_target_steps, average_distance_score, survived, can_swing_up, swing_up_time, critic_loss, actions_std
