...
def plot_group_training(sub_source_folder, save_name, y_label):
    ...
    for exp_set in exp_set_list:
        ...
        if exp_set == "S&S Reward + Residual Control":
            continue
    ...
    plot.set_xlabel("Training Steps", fontsize=12)
    plot.set_ylabel("log(Critic Loss)", fontsize=12)
    # plot.set(xlabel="Training Steps", ylabel="log(Critic Loss)")
    plot.ticklabel_format(useOffset=False, style='plain')
    plot.legend(loc='best', fontsize='large', labels=legend_list)
    plt.savefig(f'plot/{save_name}.pdf', format='pdf', bbox_inches='tight')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--folder', default="plot/NIP/data_source/critic loss plot", help='Activate usage of GPU')
    parser.add_argument('--y_label', default="Critic loss", help='Enable to write default config only')
    parser.add_argument('--save_name', default="critic loss", help='Enable to write default config only')
